...
const createTitle = data =>
  data.reduce(
    (acc, current, idx, src) =>
      `${acc}${idx > 0 && idx < src.length ? ', and' : ''} ${current.name}`,
    ''
  );
...
export default class App extends Component {
  ...
  getData(entity, parent) {
    ...
    const rawData = entities
      ...
      .map(datum => {
        ...
        return [
          ...data
            .filter(a => selectedDatasets.includes(Number(a.id)))
            .map(item => {
              const { id, data: categoryData } = item;
              const selected = selectedCategories.filter(
                i => Number(i.parent) === Number(id)
              )[0].categories;
              const data = selected
                .map(cat => categoryData.filter(o => checkForProp(o, cat)))
                .flat();
    ...
    const flatten = rawData
      .flat()
      .filter(set => set.entityId === entity)
      .map(datum => {
        ...
        const filterNested = data.map(b => {
          ...
          const filteredTags = tagset.map(item => {
            ...
            const flattenedSelectedTags = selectedTags.reduce(
              (a, c) => a.concat(c.selected),
              []
            );
            const isValid =
              findType &&
              flattenedSelectedTags.includes(findType[parent]) &&
              selectedYears.length > 0 &&
              selectedYears.includes(selectedYear);
  ...
  generateData(type, entity) {
    ...
    return this.getData(entity, parent)
      .map(node => node.data)
      .map(n => {
        return n
          .map(b => {
            ...
            return k.reduce(
              (a, c) => a + (c.value !== null ? c.value : 0.0),
              0.0
            );
          })
    ...
    return selectedDatasets.map((id, idx) => {
      ...
      return (
        <Filter
          ...
          options={options}
          onChange={selection =>
            onCategoryChange && onCategoryChange(id, selection)
          }
  ...
  generateTagFilters() {
    ...
    const set = this.getCommonTagset().filter(
      b => Object.keys(b)[0] !== 'Year'
    );
    const filters = set.map((set, idx) => {
      ...
      const options = set[title].map(option => {
        return {
          label: option,
          value: option,
          default: selectedTags.find(
            i => i.selected.includes(option) && i.parent === title
          )
        };
      });
      ...
      return (
        <Filter
          ...
          title={title}
          options={options}
          onChange={sel => this.onTagsetChange(title, sel)}
        ></Filter>
      );
    });
  ...
  getSeries() {
    ...
    const series = selectedTags
      .filter(tagset => tagset.parent !== 'Year')
      .map((item, idx) => {
        ...
        return selected.map((tag, edx) => {
          ...
          const data = entities.map(entity =>
            this.generateData({ parent, tag }, entity.id)
          );
          return {
            name: tag,
            type: 'column',
      ...
      .flat()
      .map((series, idx) => ({ ...series, stack: idx }));
    console.log(series);
    return series;
  }
  ...
  updateOptions() {
    ...
    return {
      ...
      title: {
        text: `${createTitle(titleSet)}, across ${
      ...
      <Fragment>
        <Filters>
          <Filter
            ...
            onChange={selection =>
              onDatasetChange && onDatasetChange(selection)
            }
          />
        ...
        {selectedDatasets.length > 2 && (
          <span>Please select a maximum of two datasets</span>
        )}
        <Charts>
          <HighchartsReact
        ...
        <Debug>
          ...
          <p>
            Tags:{' '}
            {selectedTags.map(e => `${e.parent}: [${e.selected.join(', ')}]`)}
          </p>
          <p>
            Categories:{' '}
            {selectedCategories.map(
              e => `${e.parent}: [${e.categories.join(', ')}]`
            )}
          </p>
          <p>
            Datasets:{' '}
            {selectedDatasets
              .map(e => this.getSelectedDataset(e).name)
              .join(', ')}
          </p>
        </Debug>
      </Fragment>
