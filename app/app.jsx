...
const selectedTags = [
  { parent: 'Engagement', selected: ['Total', 'Ongoing', 'Non-Ongoing'] }
];
...
class Container extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    const data = Promise.all(fetches)
      .then(data => {
        this.setState({
          entityData: data
        });
  ...
  onChange(selection) {
    ...
    if (isDeleting) {
      ...
      this.setState(
        {
          selectedCategories: [
            ...selectedCategories.filter(i => i.parent !== parent)
          ],
          selectedDatasets: [
            ...selectedDatasets.filter(i => Number(i) !== parent)
          ]
        },
        () => {
    ...
    if (isAdding) {
      ...
      const newDatasets = selectionIds.map(id =>
        datasets.find(i => Number(i.id) === Number(id))
      );
      const selectedIds = newDatasets.map(d => Number(d.id));
      ...
      selectedTags: [
        ...selectedTags.filter(tag => tag.parent !== parent),
        {
          ...
          selected:
            selected === null
              ? []
              : selected.reduce((a, c) => a.concat(c.value), [])
        }
      ]
    });
  }
  ...
  render() {
    ...
    return (
      <Fragment>
        {!isLoading && (
          <App
            onYearChange={s => this.onYearChange(s)}
            years={years}
